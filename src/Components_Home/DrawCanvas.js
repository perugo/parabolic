...
var totalPointsX;
var freq;
export const DrawCanvas = ({ drawData, rect, setSetting }) => {
  ...
  useCanvasAndWidthHeight(layoutWrapperRef, canvasRefs, ctxRefs, setWidth, setHeight, rect);
  ...
  useEffect(() => {
    ...
    if (checker_NOCHANGE(drawData, prevDrawDataRef.current) && compare_RectNOCHANGE(prevRect, width, height)) {
      ...
      if (inputTotalPointsX !== totalPointsX) {
        ...
        setSetting({ ...drawData.setting, totalPointsX: totalPointsX });
      }
