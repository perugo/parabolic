...
function Graph({ setting, amplitudeScaler }) {
  ...
  useEffect(() => {
    ...
    const { freq: inputFreq,nx: inputNx, fieldX: inputFieldX } = setting;
    setFreq(inputFreq);
    setNx(inputNx);
  ...
  },[freq,nx,fieldX,slope,shift,peakPosition,widthFactor,simulationNum])
  ...
  const trace1 = {
    ...
    y: Array.from({ length: simulationNum + 1 }, (_, i) => Math.sin(1.27279 * 3.141 * i * fieldX * freq / (3e8 * nx)) *
...
export function checker_SETTING(obj) {
  if (!obj) return false;
  const settingFields = ['fieldX', 'fieldY', 'nx',
  'totalPointsX', 'focalDistance','freq', 'theta'];
